...
def run_python_sandboxed(code: str,
                         timeout: int = 30,
                         memory_mb: int = 256,
                         cpu_seconds: int = 30) -> str:
    """Run Python code in an isolated sandbox with resource limits."""
    result = run_sandboxed_python(code, timeout=timeout,
...
SANDBOX_DEFINITIONS = [
    {"type": "function", "function": {
        ...
        "description": (
            "Run Python code in an isolated sandbox with CPU/memory limits "
            "(256MB, 30s), no network. Safer than run_python."
        ),
        "parameters": {"type": "object", "properties": {
            "code": {"type": "string", "description": "Python code to execute"},
            "timeout": {"type": "integer", "description":
                "Max execution time in seconds (default 30)"},
            "memory_mb": {"type": "integer", "description":
                "Memory limit in MB (default 256)"},
            "cpu_seconds": {"type": "integer", "description":
                "CPU time limit in seconds (default 30)"},
        }, "required": ["code"]}}},
    {"type": "function", "function": {
        "name": "run_command_sandboxed",
        "description": (
            "Run a shell command in an isolated sandbox with CPU/memory limits. "
            "No network by default. Safer than run_terminal_command."
        ),
        "parameters": {"type": "object", "properties": {
            ...
            "cpu_seconds": {"type": "integer", "description":
                "CPU time limit in seconds (default 30)"},
            "allow_network": {"type": "boolean", "description":
